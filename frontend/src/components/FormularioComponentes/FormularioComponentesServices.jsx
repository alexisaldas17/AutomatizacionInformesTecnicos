...
export const obtenerUsuarioPorNombre = async (nombre) => {
  const response = await fetch(`http://172.20.70.113:3000/api/usuarios/detalle?nombre=${encodeURIComponent(nombre)}`);
...
export const obtenerEquipoPorNombre = async (nombre) => {
  const response = await fetch(`http://172.20.70.113:3000/api/equipos/detalle?nombre=${encodeURIComponent(nombre)}`);
